...
func annotationCount(t *testing.T, inFile string) int {
	...
	count, _, err := core.ListAnnotations(annots)
	if err != nil {
...
func TestAddRemoveAnnotationsByObjNr(t *testing.T) {
	...
	ok, err := core.AddAnnotations(ctx, allPages, linkAnn, false)
	if err != nil || !ok {
		t.Fatalf("%s add: %v\n", msg, err)
	}
	...
	objNrs, err := core.CachedAnnotationObjNrs(ctx)
	if err != nil {
...
func TestAddAnnotationsLowLevel(t *testing.T) {
	...
	if ok, err := core.AddAnnotationsMap(ctx, m, false); err != nil || !ok {
		t.Fatalf("%s add: %v\n", msg, err)
	}
	...
	// We should have 2 annotations.
	i, _, err := core.ListAnnotations(ctx.PageAnnots)
	if err != nil || i != 2 {
		t.Fatalf("%s list: %v\n", msg, err)
	}

	// Remove all annotations.
	_, err = core.RemoveAnnotations(ctx, nil, nil, nil, false)
	if err != nil {
		t.Fatalf("%s remove: %v\n", msg, err)
	}
