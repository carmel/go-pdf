...
func Import(s string, u types.DisplayUnit) (*core.Import, error) {
	return core.ParseImportDetails(s, u)
}
...
func ImportImages(rs io.ReadSeeker, w io.Writer, imgs []io.Reader, imp *core.Import, conf *model.Configuration) error {
	...
	if rs != nil {
		...
		ctx, err = core.CreateContextWithXRefTable(conf, imp.PageDim)
	}
	if err != nil {
		return err
	}
	...
	for _, r := range imgs {
		...
		indRef, err := core.NewPageForImage(ctx.XRefTable, r, pagesIndRef, imp)
		if err != nil {
			return err
		}
