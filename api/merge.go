...
func appendTo(rs io.ReadSeeker, fName string, ctxDest *model.Context, dividerPage bool) error {
	...
	return core.MergeXRefTables(fName, ctxSource, ctxDest, false, dividerPage)
}
...
func prepDestContext(destFile string, rs io.ReadSeeker, conf *model.Configuration) (*model.Context, error) {
	...
	if conf.CreateBookmarks {
		if err := core.EnsureOutlines(ctxDest, filepath.Base(destFile), conf.Cmd == model.MERGEAPPEND); err != nil {
			return nil, err
		}
	}
...
func MergeCreateZip(rs1, rs2 io.ReadSeeker, w io.Writer, conf *model.Configuration) error {
	...
	if _, err = core.RemoveBookmarks(ctxDest); err != nil {
		return err
	}
	...
	if err := core.MergeXRefTables("", ctxSrc, ctxDest, true, false); err != nil {
		return err
	}
