...
// ExtractImagesRaw returns []core.Image containing io.Readers for images contained in selectedPages.
...
func ExtractImagesRaw(rs io.ReadSeeker, selectedPages []string, conf *model.Configuration) ([]map[int]model.Image, error) {
	...
	for i, v := range pages {
		...
		mm, err := core.ExtractPageImages(ctx, i, false)
		if err != nil {
			return nil, err
		}
...
func ExtractImagesFile(inFile, outDir string, selectedPages []string, conf *model.Configuration) error {
	...
	return ExtractImages(f, selectedPages, core.WriteImageToDisk(outDir, fileName), conf)
}

func writeFonts(ff []core.Font, outDir, fileName string) error {
	for _, f := range ff {
...
func ExtractFonts(rs io.ReadSeeker, outDir, fileName string, selectedPages []string, conf *model.Configuration) error {
	...
	ff, err := core.ExtractFormFonts(ctx)
	if err != nil {
		return err
	}
...
func ExtractPage(ctx *model.Context, pageNr int) (io.Reader, error) {
	ctxNew, err := core.ExtractPages(ctx, []int{pageNr}, false)
	if err != nil {
		return nil, err
	}
...
func ExtractContent(rs io.ReadSeeker, outDir, fileName string, selectedPages []string, conf *model.Configuration) error {
	...
	for p, v := range pages {
		...
		r, err := core.ExtractPageContent(ctx, p)
		if err != nil {
			return err
		}
		if r == nil {
